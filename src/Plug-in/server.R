...
server = function(input, output) {
  ...
  estimateCoeffs = function(xy1, xy2) {
    ...
    output$mu1OutX = render(mu1[1])
    output$mu1OutY = render(mu1[2])
    output$mu2OutX = render(mu2[1])
    output$mu2OutY = render(mu2[2])
    
    output$cov1OutX = render(cov1[1, 1])
    output$cov1OutY = render(cov1[2, 2])
    output$cov2OutX = render(cov2[1, 1])
    output$cov2OutY = render(cov2[2, 2])
